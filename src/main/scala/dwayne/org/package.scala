...
enum PairedSeparator(val left: String, val right: String) {
  ...
  case TriangularBrackets extends PairedSeparator(left = "<", right = ">")
}
...
package org {
  ...
  val titleTransformer = new IsoStringTransformer[Task] {
    def consume(s: String) = s.strip.split("\n").toList match {
      ...
      case Nil => error(s, "Got empty string")
      case _   => error(s, f"Unable to parse title string: $s")
    }
  }

  def timeTransformer(name: String, separator: PairedSeparator) = new StringTransformer[Unit, ScheduledType] {
  ...
  def scheduledTransformer = timeTransformer("SCHEDULED", PairedSeparator.TriangularBrackets)
    .lift[Task, Task](_ => (), b => Task.scheduledLense.replace(b.some))
    .asIso

  def deadlineTransformer =
    timeTransformer("DEADLINE", PairedSeparator.TriangularBrackets)
      .lift[Task, Task](_ => (), b => Task.deadlineLense.replace(b.some))
      .asIso

  def closedTransformer =
    timeTransformer("CLOSED", PairedSeparator.Brackets)
      .lift[Task, Task](_ => (), b => Task.closedLense.replace(b.some))
      .asIso

  val singlePropertyTransformer = new IsoStringTransformer[Task] {
    def consume(s: String) = {
  ...
  def stringConsumptionTransformer[T](name: String) =
    new IsoStringTransformer[T] {
      def consume(s: String) = {
        val regex = s"$name".r
        s.strip.split("\n").toList match {
          case regex() :: rest => (identity[T].valid, rest.mkString("\n"))
          case _               => error(s, f"Expected '$name', but didn't find it in '${s.strip}'")
        }
      }
    }

  val bodyTransformer = new IsoStringTransformer[Task] {
    def consume(s: String) = {
      val regex        = "^\\*".r
      val (body, rest) = s.strip.split("\n").toList.span(!regex.matches(_))
      (Task.bodyLense.replace(body.mkString("\n")).valid, rest.mkString("\n"))
    }
  }

  def parallelTransformer[T](transformers: IsoStringTransformer[T]*) = new IsoStringTransformer[T] {
  ...
  val taskTransformer = titleTransformer
    .andThen(
      parallelTransformer(
        scheduledTransformer,
        deadlineTransformer,
        closedTransformer,
      ).repeatedWhileValid
    )
    .andThen(stringConsumptionTransformer[Task](":PROPERTIES:"))
    .andThen(singlePropertyTransformer.repeatedWhileValid)
    .andThen(stringConsumptionTransformer[Task](":END:"))
    .andThen(bodyTransformer)
